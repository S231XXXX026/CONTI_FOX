...
[rospub, msg] = rospublisher('test_image_1', 'sensor_msgs/Image');

% Send image in a loop
while true
    % Assign message fields based on the provided 'sensor_msg/Image' structure
   
   if isempty(msg.Header)
        msg.Header = rosmessage('std_msgs/Header');
    end
    
    % Set current time
    msg.Header.Stamp = rostime('now');
    
    % You can set FrameId if needed, or leave it as an empty string
    % msg.Header.FrameId = ''; '';  % Set the frame ID to a meaningful value or leave as an empty string
